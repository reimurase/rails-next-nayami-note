...
describe("RoadmapCreateSheet", () => {
  test("isOpen=false のときは何も表示しない", () => {
    render(
      <RoadmapCreateSheet concernId={1} isOpen={false} onClose={jest.fn()} onCreated={jest.fn()} />
    );
  ...
  test("isOpen=true のときは見出しとフォームが表示される", () => {
    render(
      <RoadmapCreateSheet concernId={1} isOpen={true} onClose={jest.fn()} onCreated={jest.fn()} />
    );
  ...
  test("✕ボタンを押したときに onClose が呼ばれる", () => {
    ...
    render(
      <RoadmapCreateSheet concernId={1} isOpen={true} onClose={handleClose} onCreated={jest.fn()} />
    );
