...
describe("IssueCreateSheet", () => {
  test("isOpen=false のときは何も表示しない", () => {
    render(
      <IssueCreateSheet concernId={1} isOpen={false} onClose={jest.fn()} onCreated={jest.fn()} />
    );
  ...
  test("isOpen=true のときは見出しとフォームが表示される", () => {
    render(
      <IssueCreateSheet concernId={1} isOpen={true} onClose={jest.fn()} onCreated={jest.fn()} />
    );
  ...
  test("✕ボタンを押したときに onClose が呼ばれる", () => {
    ...
    render(
      <IssueCreateSheet concernId={1} isOpen={true} onClose={handleClose} onCreated={jest.fn()} />
    );
  ...
  test("IssueForm 内で onCreated が呼ばれたら、親から渡した onCreated も呼ばれる", () => {
    ...
    render(
      <IssueCreateSheet concernId={1} isOpen={true} onClose={jest.fn()} onCreated={handleCreated} />
    );
