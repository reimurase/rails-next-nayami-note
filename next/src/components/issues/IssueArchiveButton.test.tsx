// src/components/issues/IssueArchiveButton.test.tsx
...
describe("IssueArchiveButton 正常系", () => {
  beforeEach(() => {
  ...
  test("ライブラリボタンをクリックするとアーカイブAPIが呼ばれ、onArchiveChangedも呼ばれる", async () => {
    ...
    render(
      <IssueArchiveButton issueId={1} archivedAt={null} onArchiveChanged={onArchiveChangedMock} />
    );
  ...
  test("移動中はボタンが '移動中...' に変わる", async () => {
    ...
    render(<IssueArchiveButton issueId={1} archivedAt={null} />);
...
describe("IssueArchiveButton 異常系", () => {
  test("ライブラリへの移動が失敗したらエラーが表示され、onArchiveChanged は呼ばれず、ボタンは元に戻る", async () => {
    const consoleSpy = jest.spyOn(console, "error").mockImplementation(() => {});

    mockedIssueApi.archiveIssue.mockRejectedValueOnce(new Error("archiveIssue failed"));
