...
ログインはIP単位（3分/10回）とメールアドレス単位（15分/10回）の二段階で制限している。

IP単位の制限は、Cloudflare 経由のホスティング環境（Render）では `request.remote_ip` が実クライアントIPを返さないため機能しない。そのためメールアドレス単位の制限を主防御層として加えた。パスワードリセット申請も同様に二段階で制限している。
